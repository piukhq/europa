...
RUN addgroup --gid 1550 apps && \
 ...
 apt-get update && \
 apt-get -y install rsync git git-core python3 python3-pip libpq-dev libxml2-dev libxslt1-dev python3-dev nginx curl tzdata locales&& \
 ...
 rsync -a --remove-source-files /usr/local/src/europa/docker_root/ / && \
 pip3 install --upgrade pip && \
 pip3 install uwsgi pipenv && \
 cd /usr/local/src/europa/ && \
