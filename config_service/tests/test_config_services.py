...
class TestConfigService(TestCase):
    def setUp(self):
        ...
        self.inbound_service = SecurityService.objects.create(
            request_type=SecurityService.INBOUND_REQUEST, type=SecurityService.RSA_SECURITY, configuration=self.config
        )

        self.outbound_service = SecurityService.objects.create(
            request_type=SecurityService.OUTBOUND_REQUEST, type=SecurityService.RSA_SECURITY, configuration=self.config
        )
    ...
    def test_retrieve_config_view(self):
        ...
        resp = self.client.get(reverse("configuration"), {"merchant_id": "fake-merchant", "handler_type": 0})
    ...
    def test_incorrect_handler_returns_error_response(self):
        resp = self.client.get(reverse("configuration"), {"merchant_id": "fake-merchant", "handler_type": 123})
    ...
    def test_missing_config_returns_error_response(self):
        resp = self.client.get(reverse("configuration"), {"merchant_id": "fake-merchant", "handler_type": 2})
